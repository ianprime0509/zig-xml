...
pub const Streaming = struct {
    ...
    // TODO: eliminate this and use reader's buffer directly
    buf: []u8,
    pos: usize,
    avail: usize,
    gpa: Allocator,
    ...
    fn move(reader: *Reader, advance: usize, len: usize) MoveError!void {
        ...
        self.discardRead();
        self.fillBuffer(len) catch |err| switch (err) {
            error.ReadFailed => return error.ReadFailed,
            error.OutOfMemory => return error.ReadFailed, // TODO: store this information somewhere
        };
    ...
    fn fillBuffer(reader: *Streaming, target_len: usize) !void {
        ...
        read: switch (reader.state) {
            .start => {
                reader.read_len = try reader.in.readSliceShort(&reader.read);
                if (std.mem.startsWith(u8, reader.read[0..reader.read_len], "\xFE\xFF")) {
                    reader.state = .utf16be;
                } else if (std.mem.startsWith(u8, reader.read[0..reader.read_len], "\xFF\xFE")) {
                    reader.state = .utf16le;
                } else {
                    reader.state = .utf8;
                    // Since doc.read.len == min_buf_len, we know we can copy
                    // all of the read buffer into the document buffer.
                    @memcpy(reader.buf[0..reader.read_len], reader.read[0..reader.read_len]);
                    reader.avail += reader.read_len;
                }
            ...
            .utf16be, .utf16le => {
                const endian: std.builtin.Endian = if (reader.state == .utf16be) .big else .little;
                while (reader.avail < reader.buf.len) {
                    const n = try reader.in.readSliceShort(reader.read[reader.read_len..]);
                    if (n == 0 and reader.transcode_buf_len == 0 and reader.read_len == 0) break;
                    reader.read_len += n;
                    reader.transcodeUtf16(endian);
                }
            },
        }
    }
    ...
    fn transcodeUtf16(reader: *Streaming, endian: std.builtin.Endian) void {
        ...
        while (reader.avail < reader.buf.len and read_pos < reader.read_len) {
            const cp: u21, const src_len: usize = next_cp: {
                if (read_pos + 1 == reader.read_len) {
                    ...
                    break :next_cp .{ 0xD800 + @as(u16, reader.read[read_pos]), 1 };
                }
                const u = std.mem.readInt(u16, reader.read[read_pos..][0..2], endian);
                if (std.unicode.utf16IsHighSurrogate(u)) {
                    // High surrogate
                    if (read_pos + 4 > reader.read_len) {
                        // We might have more input; try reading more.
                        if (reader.read_len == reader.read.len) break;
                        // Otherwise, this is just an unpaired surrogate.
                        break :next_cp .{ u, 2 };
                    }
                    const low = std.mem.readInt(u16, reader.read[read_pos + 2 ..][0..2], endian);
        ...
        std.mem.copyForwards(u8, &reader.read, reader.read[read_pos..]);
        reader.read_len -= read_pos;
    }
...
pub fn attributeValue(reader: *Reader, n: usize) Allocator.Error![]const u8 {
    ...
    return reader.scratch.getWritten();
}
...
test attributeValueWrite {
    ...
    try expectEqualStrings("1", out.getWritten());

    out.clearRetainingCapacity();
    try reader.attributeValueWrite(1, &out.writer);
    try expectEqualStrings("2", out.getWritten());

    out.clearRetainingCapacity();
    try reader.attributeValueWrite(2, &out.writer);
    try expectEqualStrings("1 & 2", out.getWritten());
}
...
test commentWrite {
    ...
    try expectEqualStrings(" Hello, world! ", out.getWritten());
}
...
test piDataWrite {
    ...
    try expectEqualStrings("pi-data", out.getWritten());
}
